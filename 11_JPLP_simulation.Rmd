...
ggplot() + 
  ...
  geom_point(data = filter(dlambda, Type == 'JPLP' & x %in% t_trip),
             aes(x = x, y = y), color = '#e41a1c', size = 2, shape = 1) +
  # Vertical lines indicating stops
  geom_vline(xintercept = t_trip, linetype = "dashed", color = '#4daf4a') +
  labs(x = 'Time (hours)', 
       y = 'Intensity function (Lambda)') + 
  theme_minimal() + 
  guides(color = FALSE) +
  geom_point(data = d_event, aes(x = t_events, y = y), 
             alpha = 0.8, shape = 4, color = 'red', size = 2) + 
