...
ggplot(data = dLambda) + 
  geom_line(aes(x = x, y = y, group = Type, color = Type)) + 
  scale_color_manual(values = c(PLP = "#0B775E", JPLP = "#F2300F")) + 
  geom_vline(xintercept = t_trip, linetype = "dashed") +
  labs(x = 'Time (hours)', 
       y = 'Mean function (Lambda)') + 
...
sim_mul_jplp = function(kappa = 0.8, beta = 1.5, theta = 2, n_shift = 10)
{
  ...
  for (i in 1:n_shift) {
    ...
    n_stop = get_n_stop()
    sim_t_trip = runif(n_stop, min = 1, max = sim_tau - 1)
...
```{r fig.cap='Simulated events (red cross) in different shifts'}
...
plot_jplp = function(dt){
  p = ggplot() + 
    ...
    geom_point(data = dt$trip_time, aes(x = trip_time, y = shift_id), 
               shape = 3, color = 'blue', fill = 'blue', size = 3.5) + 
